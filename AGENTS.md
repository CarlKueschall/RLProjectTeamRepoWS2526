...
**Best Checkpoint: 336.8k gradient steps** (`best_selfplay_336k.pth`)
...
Achieved using a **2-phase training approach** (~59 hours total):
1. **Phase 1**: Mixed opponents only (50% weak / 50% strong), no self-play (~39h, 0→~394k steps)
2. **Phase 2**: Resume from 180k checkpoint, add self-play (pool 20, PFSP variance, 50% anchor) (~20h, 180k→357k steps)
...
2. **Main Loop** (per iteration):
   - **Gradient Updates** (replay_ratio times):
     ...
     - Train world model (reconstruction, reward, KL)
...
## Recommended Training Approach (2-Phase)

The current best benchmark performance (93.5% combined win rate) was achieved with this 2-phase setup:

### Phase 1: Mixed Opponents Only
```bash
cd 02-SRC/DreamerV3
python3 train_hockey.py \
    --seed 42 \
...
### Phase 2: Resume from 180k + Add Self-Play
```bash
python3 train_hockey.py \
    --seed 42 \
...
**Key insights:**
1. Mixed weak+strong opponents are enough to saturate fixed-bot performance.
2. Self-play should be introduced from a strong checkpoint to improve robustness, not early bootstrap.
3. DreamSmooth and Two-Hot remain core for sparse reward learning stability.
4. Keep actor LR <= critic LR and avoid aggressive late-stage LR changes without evidence.
